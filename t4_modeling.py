...
def main():
    ...
    # load data, have to be transformed to size divisible by 32 for Unet input
    ...
    with rasterio.open(raster_path, "r+", driver="JP2OpenJPEG") as src:
        X = src.read()
        # transform to size divisible by 32
        X = X[:, :10976, :10976]
        X = X.transpose(1, 2, 0)
        X_list.append(X)
...
def build_unet_model(): # inputs
   inputs = layers.Input(shape=(10976, 10976, 3))
